...
class NeuSSystem(BaseSystem):
    ...
    def prepare(self):
        ...
        # Add in s3im loss
        self.lambda_s3im = self.config.system.loss.get('lambda_s3im', 0)
        self.s3im_kernel_size = self.config.system.loss.get('s3im_kernel_size', 4)
        self.s3im_stride = self.config.system.loss.get('s3im_stride', 4)
        self.s3im_repeat_time = self.config.system.loss.get('s3im_repeat_time', 10)
        self.s3im_patch_height = self.config.system.loss.get('s3im_patch_height', 64)
        self.enable_s3im = False
        if self.lambda_s3im > 0:
        # s3im loss, not works very well
            self.s3im_loss = S3IM(s3im_kernel_size=self.s3im_kernel_size, s3im_stride=self.s3im_stride, s3im_repeat_time=self.s3im_repeat_time, s3im_patch_height=self.s3im_patch_height)
            print(f"Using S3IM loss at weight {self.lambda_s3im}, kernel size {self.s3im_kernel_size}, stride {self.s3im_stride}, repeat_time {self.s3im_repeat_time}, patch {self.s3im_patch_height}!")

        self.lambda_orientation = self.C(self.config.system.loss.lambda_orientation)
        if self.lambda_orientation > 0:
            print(f"Using Orientation loss initializes at scale {self.lambda_orientation}")
            
        self.sdf_con = self.config.system.loss.get('lambda_sdf_con', 0)
        self.curv_warmup = self.config.system.loss.get('curvature_warmup_steps', 0)
        
        if self.sdf_con > 0:
            print(f"Using Consistnt Constraint for SDF at scale {self.sdf_con}")
        
    def forward(self, batch):
        if self.config.model.texture.name == 'volume-radiance-ASG-E':
    ...
    def training_step(self, batch, batch_idx):
        ...
        # adaptive ray loss term from RaNeuS
        with torch.no_grad():
        ...
        # Geometric constraint loss term from RaNeuS
        if self.C(self.config.system.loss.lambda_surface_bias) > 0:
            ...
            self.log('train/loss_geo_bias', loss_bias)
            loss += loss_bias * self.C(self.config.system.loss.lambda_surface_bias) 
          
        if self.sdf_con > 0.0:
            loss_con = out['sdf_consis_constraint'].mean()
            # print(loss_eikonal)
            self.log('train/loss_consistent', loss_con)
            loss += loss_con * self.C(self.config.system.loss.lambda_sdf_con)
        ...
        # orientation loss proposed in refnerf
        if self.lambda_orientation > 0:
            w = out['weights']
            n_dot_v = out['n_dot_v']
            zero = torch.tensor(0.0, dtype=torch.float32)
            loss_orientation = torch.mean((w * torch.minimum(zero, n_dot_v)**2).sum(dim=-1))
            loss += loss_orientation * self.C(self.config.system.loss.lambda_orientation)
            self.log('train/loss_orientation', loss_orientation)
        ...
        if self.lambda_s3im > 0 and self.enable_s3im:
            loss_rgb_s3im = self.s3im_loss(out['comp_rgb_full'], batch['rgb'])
            self.log('train/loss_rgb_s3im', loss_rgb_s3im)
            loss += loss_rgb_s3im * self.C(self.config.system.loss.lambda_s3im)
        ...
        if self.train_num_rays % self.s3im_patch_height == 0:
            self.enable_s3im = True
        if self.train_num_rays % self.s3im_patch_height != 0:
            self.enable_s3im = False
            # prevent the fluctuation of train_num_rays
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.save_image_grid(f"it{self.global_step}-{batch['index'][0].item()}.png", [
            ...
            {'type': 'rgb', 'img': batch['normal_pseudo'].view(H, W, 3), 'kwargs': {'data_format': 'HWC', 'data_range': (-1, 1)}}
        ] if self.config.model.geometry.name == 'volume-sdf-laplace-fusion-spa' else [])
        )
        
        if self.config.model.texture.name == 'volume-radiance-ide':
            self.save_image_grid(f"it{self.global_step}-{batch['index'][0].item()}_3_vis.png", [
            {'type': 'rgb', 'img': out['comp_base_full'].view(H, W, 3), 'kwargs': {'data_format': 'HWC'}},
            {'type': 'rgb', 'img': out['comp_ref_full'].view(H, W, 3), 'kwargs': {'data_format': 'HWC'}},
            {'type': 'rgb', 'img': out['comp_light_full'].view(H, W, 3), 'kwargs': {'data_format': 'HWC'}}] 
        )
        
        if self.config.model.texture.name == 'volume-radiance-uni' or self.config.model.texture.name == 'volume-radiance-ASG' or self.config.model.texture.name == 'volume-radiance-ASG-SG' or self.config.model.texture.name == 'volume-radiance-ASG-E':
            self.save_image_grid(f"it{self.global_step}-{batch['index'][0].item()}_3_vis.png", [
            {'type': 'rgb', 'img': out['comp_base_full'].view(H, W, 3), 'kwargs': {'data_format': 'HWC'}},
            {'type': 'rgb', 'img': out['comp_ref_full'].view(H, W, 3), 'kwargs': {'data_format': 'HWC'}}] 
        )

...
def get_encoding(n_input_dims, config):
    ...
    if config.get('ide_enable', False):
        from .ide_encoder import IntegratedDirEncoder
        encoding_ide = IntegratedDirEncoder(input_dim=n_input_dims, deg_view=config.ide_sh_degree)
        # Default set to 5
        encoding_ide = CompositeEncoding(encoding_ide, include_xyz=False, xyz_scale=2., xyz_offset=-1.)
        return encoding, encoding_ide
...
class LipchitzMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(self.layers)):
            ...
            x = torch.nn.functional.linear(x.float(), weight, bias)
    ...
    def make_linear(self, dim_in, dim_out, is_first, is_last):
        ...
        self.weights_per_layer.append(layer.weight)
        self.biases_per_layer.append(layer.bias)
        max_w = torch.max(torch.sum(torch.abs(layer.weight), dim=1))

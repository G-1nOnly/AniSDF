...
# nerfacc modification from 0.3.3 to 0.5.0
...
class NeuSModel(BaseModel):
    ...
    def get_alpha(self, sdf, normal, dirs, dists):
        ...
        # weight initialization in HF-Neus
    ...
    def forward_bg_(self, rays, camera_indices=None):
        n_rays = rays.shape[0]
        ...
        if self.config.texture.name == 'volume-radiance-ASG-E':
    ...
    def forward_(self, rays, camera_indices=None):
        n_rays = rays.shape[0]
        rays_o, rays_d = rays[:, 0:3], rays[:, 3:6] # both (N_rays, 3)
        with torch.no_grad():
            ray_indices, t_starts, t_ends = ray_marching(
        ...
        bs, nsamples = t_ends.shape[0] // n_rays, n_rays

        if self.config.geometry.grad_type == 'finite_difference' or self.config.geometry.name == 'volume-sdf-laplace' or self.config.geometry.name == 'volume-sdf-laplace-fusion':
            sdf, sdf_grad, feature, sdf_laplace = self.geometry(positions, with_grad=True, with_feature=True, with_laplace=True)
        elif self.config.geometry.name == 'volume-sdf-laplace-fusion-con':
            sdf, sdf_grad, feature, sdf_laplace, consis_constraint = self.geometry(positions, with_grad=True, with_feature=True, with_laplace=True, with_consistent=True) 
        else:
            sdf, sdf_grad, feature = self.geometry(positions, with_grad=True, with_feature=True)
        normal = F.normalize(sdf_grad, p=2, dim=-1)
        alpha = self.get_alpha(sdf, normal, t_dirs, dists)[...,None]
        # rgb = self.texture(feature, t_dirs, normal)
        if self.config.texture.name == 'volume-radiance-ide':
            rgb, base, ref, light = self.texture(feature, t_dirs, normal)
        elif self.config.texture.name == 'volume-radiance-uni':
            rgb, base, ref = self.texture(feature, t_dirs, normal)
        elif self.config.texture.name == 'volume-radiance-ASG' or self.config.texture.name == 'volume-radiance-ASG-SG':
            rgb, base, ref = self.texture(feature, t_dirs, normal, positions)
        elif self.config.texture.name == 'volume-radiance-ASG-E':
            rgb, base, ref = self.texture(feature, t_dirs, normal, camera_indices.to(self.rank), ray_indices)
        else:
            rgb = self.texture(feature, t_dirs, normal)
        ...
        # Geometric Bias Loss Term from RaNeuS
        ...
        if self.config.texture.name == 'volume-radiance-ide':
            comp_base = accumulate_along_rays(weights, ray_indices, values=base, n_rays=n_rays)
            comp_ref = accumulate_along_rays(weights, ray_indices, values=ref, n_rays=n_rays)
            comp_light = accumulate_along_rays(weights, ray_indices, values=light, n_rays=n_rays)
        if self.config.texture.name == 'volume-radiance-uni' or self.config.texture.name == 'volume-radiance-ASG' or self.config.texture.name == 'volume-radiance-ASG-SG' or self.config.texture.name == 'volume-radiance-ASG-E':
            comp_base = accumulate_along_rays(weights, ray_indices, values=base, n_rays=n_rays)
            comp_ref = accumulate_along_rays(weights, ray_indices, values=ref, n_rays=n_rays)

        comp_normal = accumulate_along_rays(weights, ray_indices, values=normal, n_rays=n_rays)
        comp_normal = F.normalize(comp_normal, p=2, dim=-1)
        # Add in opacity restriction
        rays_fg = opacity > 0.1
        ...
        if self.training:
            ...
            if self.config.geometry.grad_type == 'finite_difference' or self.config.geometry.name == 'volume-sdf-laplace' or self.config.geometry.name == 'volume-sdf-laplace-fusion' or self.config.geometry.name == 'volume-sdf-laplace-fusion-con':
                out.update({
                    'sdf_laplace_samples': sdf_laplace
                })
            # Implementation of orientation loss in ref-NeRF
            n = normal
            v = -t_dirs
            n_dot_v = (n * v).sum(dim=-1, keepdim=True)
            out.update({
            'n_dot_v': n_dot_v.view(-1),           
            })
            # Geometric Bias
            ...
            if self.config.geometry.name == 'volume-sdf-laplace-fusion-con':
                out.update({
                    'sdf_consis_constraint': consis_constraint
                })
        ...
        if self.config.texture.name == 'volume-radiance-ide':
            out_full = {
            'comp_rgb': out['comp_rgb'] + out_bg['comp_rgb'] * (1.0 - out['opacity']),
            'comp_base': out['comp_base'] + out_bg['comp_rgb'] * (1.0 - out['opacity']),
            'comp_ref': out['comp_ref'] + out_bg['comp_rgb'] * (1.0 - out['opacity']),
            'comp_light': out['comp_light'] + out_bg['comp_rgb'] * (1.0 - out['opacity']),
            'num_samples': out['num_samples'] + out_bg['num_samples'],
            'rays_valid': out['rays_valid'] | out_bg['rays_valid']
        }
    ...
    def forward(self, rays, camera_indices=None):
        if self.training:
            if self.config.texture.name == 'volume-radiance-ASG-E':
    ...
    def export(self, export_config):
        ...
        if export_config.export_vertex_color:
            ...
            normal = F.normalize(sdf_grad, p=2, dim=-1)
            if self.config.texture.name == 'volume-radiance-ide':
                rgb, base, ref, light = self.texture(feature, -normal, normal)
            elif self.config.texture.name == 'volume-radiance-uni':
                rgb, base, ref= self.texture(feature, -normal, normal)
            elif self.config.texture.name == 'volume-radiance-ASG' or self.config.texture.name == 'volume-radiance-ASG-SG':
                rgb, base, ref = self.texture(feature, -normal, normal, mesh['v_pos'].to(self.rank))
                rgb = base
                # only extract the base(diffuse) color
            elif self.config.texture.name == 'volume-radiance-ASG-E':
                rgb, base, ref = self.texture(feature, -normal, normal, mesh['v_pos'].to(self.rank), torch.empty(0), torch.empty(0)) 
                rgb = base
                # only extract the base(diffuse) color
            else:
